...
export const listDataSourcePage = (params?: httpNs.PageData<DataSource.DataSourceSearch>) => {
  return http.get<httpNs.Response<DataSource.DataSourceInfo[]>>(`${baseUri}/listPage`, params);
};

export const listSelectDataSource = (teamId: string) => {
  return http.get<httpNs.Response<DataSource.DataSourceInfo[]>>(`${baseUri}/listSelect/${teamId}`);
};

export const listByProjectId = (projectId: string) => {
  return http.get<httpNs.Response<DataSource.DataSourceInfo[]>>(`${baseUri}/listByProjectId/${projectId}`);
};

export const listSchemaByDataSource = (dataSourceId: string) => {
  return http.get<httpNs.Response<string[]>>(`${baseUri}/listSchemaByDataSource/${dataSourceId}`);
};

export const listTableBySchema = (dataSourceId: string, schema: string) => {
  return http.get<httpNs.Response<DataSource.Table[]>>(`${baseUri}/listTableBySchema/${dataSourceId}/${schema}`);
};

export const addDataSource = (data: DataSource.DataSourceInsert) => {
  return http.post<httpNs.Response<string>>(baseUri, data);
};

export const editDataSource = (data: DataSource.DataSourceUpdate) => {
  return http.put<httpNs.Response<string>>(baseUri, data);
};

export const removeDataSource = (data: DataSource.DataSourceDelete) => {
  return http.delete<httpNs.Response<string>>(`${baseUri}/${data.dataSourceId}`);
};

export const testConnect = (data: DataSource.DataSourceSearch) => {
  return http.post<httpNs.Response<string>>(`${baseUri}/testConnect`, data);
};

export const executeSelect = (data: DataSource.SqlExecute) => {
  return http.post<httpNs.Response<Record<string, any>[]>>(`${baseUri}/executeSelect`, data);
};

export const generateTemple = (dataSourceId: string, schema: string, tableName: string, type: string) => {
  return http.post<httpNs.Response<string>>(`${baseUri}/generateTemple/${dataSourceId}/${schema}/${tableName}/${type}`);
};

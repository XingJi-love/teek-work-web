...
export declare namespace Service {
  interface ServiceInfo {
    ...
    responseTemplate: string; // 响应模板数据
...
export const listServicePage = (params?: httpNs.PageData<Service.ServiceSearch>) => {
  return http.get<httpNs.Response<Service.ServiceInfo[]>>(`${baseUri}/listPage`, params);
};

export const getServiceByServiceId = (serviceId: string) => {
  return http.get<httpNs.Response<Service.ServiceInfo>>(`${baseUri}/getByServiceId/${serviceId}`);
};

export const listSelectInProject = (projectId: string, serviceId: string) => {
  return http.get<httpNs.Response<Service.ServiceInfo[]>>(`${baseUri}/listSelectInProject/${projectId}/${serviceId}`);
};
...
export const removeService = (data: Service.ServiceDelete) => {
  return http.delete<httpNs.Response<string>>(`${baseUri}/${data.serviceId}`);
};

export const generateCol = (data: Service.ServiceUpdate) => {
  return http.post<httpNs.Response<string>>(`${baseUri}/generateCol`, data);
};

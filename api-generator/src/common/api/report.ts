...
export const getReportByServiceId = (serviceId: string) => {
  return http.get<httpNs.Response<Report.ReportInfo>>(`${baseUri}/getReportByServiceId/${serviceId}`);
};

export const editReport = (data: Partial<Report.ReportInfo>) => {
  return http.put<httpNs.Response<Report.ReportInfo>>(baseUri, data);
};

export const listReportConfig = (serviceId: string) => {
  return http.get<httpNs.Response<Report.ReportConfig>>(`${baseUri}/listReportConfig/${serviceId}`);
};

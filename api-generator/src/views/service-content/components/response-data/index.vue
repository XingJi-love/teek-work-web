...
const ns = useNamespace("response-data");
...
const tableNameList = computed(() => {
  ...
  tableList.value.forEach(item => (tableNameList = [...tableNameList, ...item.tableNameList]));
...
watch(
  () => serviceInfo.value,
  async () => {
    const { projectId, dataSourceId } = serviceInfo.value || {};
...
const handleRun = async () => {
  const sql = sqlFormRef.value?.model.selectSql;

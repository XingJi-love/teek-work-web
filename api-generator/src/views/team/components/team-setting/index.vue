...
import { useDialog, ProForm, message, HOME_NAME, useNamespace } from "teek";
import { useRouteFn } from "@/composables";
import { useUserStore } from "@/pinia";
import { editTeam, removeTeam, transferOwner } from "@/common/api/team";
import { listMembers, leaveTeam } from "@/common/api/teamMember";

const ns = useNamespace("team-setting");
...
const editConfirm = async () => {
  ...
  const res = await editTeam({ ...teamModel, id, teamId });
 ...
 * 转移团队表单配置项
 */
const transferColumns: FormColumn[] = [
  {
    prop: "ownerName",
    label: "接收人",
    el: "el-select",
    formItemProps: { rules: [{ required: true, message: "请选择 接收人" }] },
    options: async () => {
      if (members.value.length) return { data: members.value };
...
const handleTransfer = async () => {
  open({
    ...
    width: 400,
    height: 80,
    render: () => <ProForm v-model={transferModel} columns={transferColumns} />,
...
const handleExitTeam = () => {
  open({
    ...
    width: 600,
    height: 150,
    confirmText: "我已了解后果，确定退出",
    render: () => (
      <el-space fill>
        <div>退出「{teamName.value}」团队后，您将失去该团队内所有项目的访问权限。</div>
...
const exitTeamConfirm = async () => {
  const teamId = route.meta.params?.teamId;
  if (confirmInput.value !== teamName.value) {
...
const handleRemoveTeam = () => {
  open({
    ...
    width: 600,
    height: 150,
    confirmText: "我已了解后果，确定解散",
    render: () => (
      <el-space fill>
        <div>解散「{teamName.value}」团队后，该团队下的所有项目都将被同步删除，且不可恢复！</div>
        <div class="flx-center">
          <span>请输入团队名确定操作：</span>
          <el-input v-model={confirmInput.value} class="flex-1" />
        </div>

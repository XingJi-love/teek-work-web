...
export const listAppModule = (appId: string) => {
  return http.get<httpNs.Response<File.FileInfo[]>>(`${baseUri}/listAppModule/${appId}`);
};

export const listFilePage = (params: Partial<File.FileInfo>) => {
  return http.get<httpNs.Response<File.FileInfo[]>>(`${baseUri}/listPage`, params);
};
...
export const removeFile = (appId: string, fileKey: string) => {
  return http.delete<httpNs.Response<boolean>>(`${baseUri}/${appId}/${fileKey}`);
};

export const removeBatchFile = (appId: string, fileKeyList: string[]) => {
  return http.delete<httpNs.Response<boolean>>(`${baseUri}/batch/${appId}`, {}, { data: fileKeyList });
};

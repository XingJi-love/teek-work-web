import type { AxiosResponse, RequestConfig } from "@teek/http";
import type { AxiosError } from "@teek/http";
import {
  isValidURL,
  message,
  createSimpleUuid,
...
import { LOGIN_URL, agPrivateKey, agPublicKey } from "@/common/config";
import { ContentTypeEnum, createRequest } from "@teek/http";
...
export const http = createRequest({
  ...
  interceptors: {
    ...
    onRequest: (config: RequestConfig) => {
      ...
      // 开启参数加密
      if (config.isEncrypt && ["post", "put"].includes(config.method || "")) {
        // 生成一个 AES 密钥
        const aesPassword = generateAesKey();
        // base64 加密
        const aesBase64Key = encryptBase64(aesPassword);
        // AES 密钥最终以 RAS 加密
        config.headers[encryptHeader] = encryptRas(aesBase64Key, agPublicKey);
        // 数据采用 AES 加密
        config.data =
    ...
    onResponse: (response: AxiosResponse & { config: RequestConfig }) => {
      // 是否需要解密
      const encryptKey = response.headers[encryptHeader];
      if (encryptKey) {
        // AES 解密
        const base64Key = decryptRas(encryptKey, agPrivateKey);
        // base64 解码得到请求头的 AES 秘钥
        const aesPassword = decryptBase64(base64Key.toString());
        // aesPassword 解码出实际的 data
        const decryptData = decryptAes(response.data, aesPassword);
